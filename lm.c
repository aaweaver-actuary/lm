...
SEXP Cdqrls(SEXP x, SEXP y, SEXP tol, SEXP chk)
{
    ...
    // Check that the input `x` is a matrix
    ans = getAttrib(x, R_DimSymbol);
    if(check && length(ans) != 2) error(_("'x' is not a matrix"));
    
    // Extract the dimensions of the matrix and check that the dimensions of `x` and `y` match
    ...
    // Check to ensure that both x and y are numeric. Coerce them to numeric if they are not
    if (TYPEOF(x) != REALSXP) {
